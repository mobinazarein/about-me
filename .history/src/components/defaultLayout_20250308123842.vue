<template>
  <header style="width: 100%; margin-left: auto; margin-right: auto; direction: rtl;" class="main">
    <v-row no-gutters style="background-color: white;">
      <v-col cols="12" md="12" class="d-flex">
        <v-tabs v-model="tab" align-tabs="center" color="black">
          <v-tab style="font-size: 10px; color: black; font-weight: bold;" to="/home">Home</v-tab>
          <v-tab style="font-size: 10px; color: black; font-weight: bold;" to="/about">About/Contacts</v-tab>
        </v-tabs>
      </v-col>
      <v-col cols="6" md="6" class="text-left">
        <p>some thing</p>
      </v-col>
    </v-row>
  </header>
  <main>
    <router-view></router-view>
  </main>
  <footer style="position: relative;">
    <div style="text-align: center; position: absolute; left: 47%; top: 30%;">
      <a href="https://t.me/mobina_zrrr" target="_blank">
        <v-icon class="mr-3" style="cursor: pointer;">mdi-telegram</v-icon>
      </a>
      <a href="https://www.instagram.com/mobina_zrrr" target="_blank">
        <v-icon class="mr-3" style="cursor: pointer;">mdi mdi-instagram</v-icon>
      </a>
      <a href="https://github.com/mobinazarein" target="_blank">
        <v-icon class="mr-3" style="cursor: pointer;">mdi-github</v-icon>
      </a>
      <a href="mailto:mobinazarein049@gmail.com" target="_blank">
        <v-icon style="cursor: pointer;">mdi-gmail</v-icon>
      </a>
    </div>
    <img src="/footer.png" width="100%" />
  </footer>
</template>

<script>
export default {
  name: 'DefaultLayout',
  data() {
    return {
      tab: null, // مقدار اولیه برای تب
    };
  },
  watch: {
    tab(newVal) {
      // اینجا می‌توانید با تغییر تب، محتوای مربوطه را بارگذاری کنید
    }
  }
}
</script>

<style>
a {
  color: black !important;
}

body {
  background-color: #f5f6f8 !important;
  font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif !important;
}

.background-cover {
  background-color: #c3c5a5;
}

.fade-enter-active,
.fade-leave-active {
  transition: opacity 0.3s;
}

.fade-enter,
.fade-leave-to {
  opacity: 0;
}

.title {
  filter: drop-shadow(3px 1px 5px #d1bbf7);
  position: relative;
}

.main {
  background-color: #f5f6f8;
}
</style>